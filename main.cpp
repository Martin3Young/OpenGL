...
int main()
{
    ...
    unsigned char *data = stbi_load("../src/container.jpeg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            float angle = 20.0f * i;
            model = glm::rotate(model, glm::radians(angle+10)*(float)glfwGetTime(), glm::vec3(1.0f, 0.3f, 0.5f));
